...
export const Bar = styled.div<BarProps>`
  ...
  background-color: ${({ theme, $useDark, primary }) => {
    if ($useDark) return primary ? theme.colors.primary : `${theme.colors.primary}80`;
    return primary ? lightColors.primary : `${lightColors.primary}80`;
  }};
  height: 100%;
...
const StyledProgress = styled.div<StyledProgressProps>`
  position: relative;
  background-color: ${({ theme, $useDark }) => ($useDark ? theme.colors.backgroundAlt2 : lightColors.input)};
