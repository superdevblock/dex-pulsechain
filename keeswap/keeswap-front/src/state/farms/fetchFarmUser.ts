...
export const fetchFarmUserAllowances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const calls = farmsToFetch.map((farm) => {
    ...
    const tokenContractAddress = farm.token.address
    const contractAddress = farm.isTokenOnly ? tokenContractAddress : lpContractAddress
    
    return { address: contractAddress, name: 'allowance', params: [account, masterChefAddress] }
  })
