...
const deserializeFarm = (farm: SerializedFarm): DeserializedFarm => {
  const { lpAddresses, lpSymbol, pid, dual, multiplier, isCommunity, quoteTokenPriceBusd, tokenPriceBusd, isTokenOnly } = farm
...
export const usePriceBlockBusd = (): BigNumber => {
  const blockBnbFarm = useFarmFromPid(0)

  const blockPriceBusdAsString = blockBnbFarm.tokenPriceBusd

  const blockPriceBusd = useMemo(() => {
    return new BigNumber(blockPriceBusdAsString)
  }, [blockPriceBusdAsString])

  return blockPriceBusd
}

export const usePriceBnbBusd = (): BigNumber => {
  const bnbBusdFarm = useFarmFromPid(1)
  const bnbPriceBusdAsString = bnbBusdFarm.tokenPriceBusd
...
export const useTotalFarmValue = (): BigNumber => {
  ...
  for (let i = 0; i < farms.data.length; i++) {
    ...
    if (farm.lpTotalInQuoteToken) {
      ...
      }else if (farm.quoteToken.symbol === 'KEE') {
