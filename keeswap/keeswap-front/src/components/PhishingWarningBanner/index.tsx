...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://rissot.to - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/rissot.to)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text) => (
        <Text
          small
          as="span"
          bold={text === 'https://rissot.to'}
          color={text === 'https://rissot.to' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
